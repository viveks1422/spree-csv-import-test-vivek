...
class ChangeIndexesOnFriendlyIdSlugs < ActiveRecord::Migration[5.0]
  def change
    ...
    add_index :friendly_id_slugs, %i[slug sluggable_type], length: { name: 100, slug: 20, sluggable_type: 20 }
    add_index :friendly_id_slugs, %i[slug sluggable_type scope], length: { name: 100, slug: 20, sluggable_type: 20, scope: 20 }, unique: true
  end
end
