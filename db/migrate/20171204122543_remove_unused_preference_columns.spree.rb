# frozen_string_literal: true

# This migration comes from spree (originally 20120929093553)
class RemoveUnusedPreferenceColumns < ActiveRecord::Migration[4.2]
  def change
    # Columns have already been removed if the application was upgraded from an older version, but must be removed from new apps.
    if ApplicationRecord.connection.column_exists?(:spree_preferences, :name)
      remove_column :spree_preferences, :name
    end
    if ApplicationRecord.connection.column_exists?(:spree_preferences, :owner_id)
      remove_column :spree_preferences, :owner_id
    end
    if ApplicationRecord.connection.column_exists?(:spree_preferences, :owner_type)
      remove_column :spree_preferences, :owner_type
    end
  end
end
