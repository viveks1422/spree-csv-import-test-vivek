...
class AddMissingIndexesOnSpreeTables < ActiveRecord::Migration[4.2]
  def change
    ...
    if data_source_exists?(:spree_products_promotion_rules) && !index_exists?(:spree_products_promotion_rules,
                                                                              %i[promotion_rule_id product_id],
    ...
    if data_source_exists?(:spree_orders_promotions) && !index_exists?(:spree_orders_promotions, %i[promotion_id order_id])
      add_index :spree_orders_promotions, %i[promotion_id order_id]
    end
    ...
    if data_source_exists?(:spree_taxons_prototypes) && !index_exists?(:spree_taxons_prototypes, %i[prototype_id taxon_id])
      add_index :spree_taxons_prototypes, %i[prototype_id taxon_id]
    end
